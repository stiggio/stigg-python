...
from ...types.v1 import feature_list_features_params, feature_create_feature_params, feature_update_feature_params
...
from ...types.v1.feature import Feature
from ...types.v1.feature_list_features_response import FeatureListFeaturesResponse
...
class FeaturesResource(SyncAPIResource):
    ...
    def archive_feature(
        ...
        *,
        ...
        return self._post(
            path_template("/api/v1/features/{id}/archive", id=id),
    ...
    def retrieve_feature(
        ...
        *,
        ...
        if not id:
            raise ValueError(f"Expected a non-empty value for `id` but received {id!r}")
        return self._get(
            path_template("/api/v1/features/{id}", id=id),
            options=make_request_options(
                extra_headers=extra_headers, extra_query=extra_query, extra_body=extra_body, timeout=timeout
            ),
            cast_to=Feature,
    ...
    def unarchive_feature(
        ...
        *,
        ...
        if not id:
            raise ValueError(f"Expected a non-empty value for `id` but received {id!r}")
        return self._post(
            path_template("/api/v1/features/{id}/unarchive", id=id),
            options=make_request_options(
                extra_headers=extra_headers, extra_query=extra_query, extra_body=extra_body, timeout=timeout
            ),
            cast_to=Feature,
